...
===== 1. Authenticate to the SUSE Application Collection

From a system connected to the internet, authenticate to the SUSE Application Collection:
...
Replace `<USERNAME>` with your username and `<TOKEN_OR_SECRET>` with your access token or service account secret. For more information, see the https://docs.apps.rancher.io/get-started/authentication[SUSE Application Collection Authentication documentation].

===== 2. Fetch the cert-manager Chart

Fetch the cert-manager chart from the SUSE Application Collection:
...
This will download the chart as a `.tgz` file. Note: The chart images are already configured to pull from the SUSE Application Collection registry, but you will need to mirror these images to your private registry for air-gapped installations. See the xref:installation-and-upgrade/other-installation-methods/air-gapped/publish-images.adoc[Prepare your Private Registry] documentation.
...
. Install cert-manager. Note: Update the image repositories to point to your private registry where you have mirrored the cert-manager images.
+
[,plain]
----
 helm install cert-manager ./cert-manager-v1.13.1.tgz \
