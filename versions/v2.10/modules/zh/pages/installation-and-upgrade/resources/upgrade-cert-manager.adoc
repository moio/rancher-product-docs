...
. 对 SUSE Application Collection 进行身份验证
+
[,plain]
----
...
----
+
将 `<USERNAME>` 替换为你的用户名，将 `<TOKEN_OR_SECRET>` 替换为你的访问令牌或服务账号密码。有关更多信息，请参阅 https://docs.apps.rancher.io/get-started/authentication[SUSE Application Collection 身份验证文档]。
...
. 在可以连接互联网的系统中，对 SUSE Application Collection 进行身份验证
+
[,plain]
----
 helm registry login dp.apps.rancher.io -u <USERNAME> -p <TOKEN_OR_SECRET>
