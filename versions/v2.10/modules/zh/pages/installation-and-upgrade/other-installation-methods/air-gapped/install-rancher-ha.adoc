...
===== 1. 对 SUSE Application Collection 进行身份验证
...
这将下载 chart 为 `.tgz` 文件。注意：chart 镜像已配置为从 SUSE Application Collection 镜像库拉取，但你需要将这些镜像镜像到你的私有镜像仓库以便离线安装。请参阅 xref:installation-and-upgrade/other-installation-methods/air-gapped/publish-images.adoc[准备私有镜像仓库]文档。
...
. 安装 cert-manager。注意：更新镜像仓库以指向你已镜像 cert-manager 镜像的私有镜像仓库。
+
[,plain]
----
