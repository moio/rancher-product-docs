...
以下说明使用来自 https://apps.rancher.io/applications/cert-manager[SUSE Application Collection] 的 cert-manager。有关 Application Collection 的更多信息，请参阅https://docs.apps.rancher.io/[文档]。
...
在安装 cert-manager 之前，你需要对 SUSE Application Collection OCI 镜像库进行身份验证。此镜像库需要对 Helm 操作和拉取容器镜像进行身份验证。
...
----
+
将 `<USERNAME>` 替换为你的用户名，将 `<TOKEN_OR_SECRET>` 替换为你的访问令牌或服务账号密码。有关身份验证的更多信息，请参阅 https://docs.apps.rancher.io/get-started/authentication[SUSE Application Collection 身份验证文档]。

. 创建用于从 Application Collection 拉取镜像的密文：
+
----
...
----
+
此密文将用于从 Application Collection 镜像库拉取 cert-manager 容器镜像。
...
# 从 SUSE Application Collection 安装 cert-manager Helm Chart
...
*添加 TLS 密文（千万不要遗漏此步骤）*：现在 Rancher 已经完成部署，你还需要参考xref:./resources/tls-secrets.adoc[添加 TLS 密文]发布证书文件，以便 Rancher 和 Ingress Controller 可以使用它们。
--
======

Rancher Chart 有许多选项，用于为你的具体环境自定义安装。以下是一些常见的高级方案：

* xref:./references/helm-chart-options.adoc#_http_代理[HTTP 代理]
* xref:./references/helm-chart-options.adoc#_私有仓库和离线安装[私有容器镜像仓库]
* xref:./references/helm-chart-options.adoc#_外部_tls_终止[外部负载均衡器上的 TLS 终止]

如需获取完整的选项列表，请参见 xref:./references/helm-chart-options.adoc[Chart 选项]。
...
如果遇到任何问题，请参见xref:./troubleshooting/troubleshooting.adoc[故障排除]。
