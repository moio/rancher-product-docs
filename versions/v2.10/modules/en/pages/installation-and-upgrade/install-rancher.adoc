...
If you want to externally terminate SSL/TLS, see xref:./references/helm-chart-options.adoc#_external_tls_termination[TLS termination on an External Load Balancer]. As outlined on that page, this option does have additional requirements for TLS verification.
====


There are three recommended options for the source of the certificate used for TLS termination at the Rancher server:

* *Rancher-generated TLS certificate:* In this case, you will need to install `cert-manager` into the cluster. Rancher utilizes `cert-manager` to issue and maintain its certificates. Rancher will generate a CA certificate of its own, and sign a cert using that CA. `cert-manager` is then responsible for managing that certificate. No extra action is needed when `agent-tls-mode` is set to strict. More information can be found on this setting in xref:./references/tls-settings.adoc#_agent_tls_enforcement[Agent TLS Enforcement].
...
____
You should skip this step if you are bringing your own certificate files (option `ingress.tls.source=secret`), or if you use xref:./references/helm-chart-options.adoc#_external_tls_termination[TLS termination on an external load balancer].
____
...
The following instructions use cert-manager from the https://apps.rancher.io/applications/cert-manager[SUSE Application Collection]. For more information about the Application Collection, see the https://docs.apps.rancher.io/[documentation].
...
[discrete]
==== Authenticate to the SUSE Application Collection

Before installing cert-manager, you need to authenticate to the SUSE Application Collection OCI registry. This registry requires authentication for both Helm operations and for pulling container images.

. Authenticate to the Helm registry:
+
----
helm registry login dp.apps.rancher.io -u <USERNAME> -p <TOKEN_OR_SECRET>
----
+
Replace `<USERNAME>` with your username and `<TOKEN_OR_SECRET>` with your access token or service account secret. For more information on authentication, see the https://docs.apps.rancher.io/get-started/authentication[SUSE Application Collection Authentication documentation].

. Create a secret for pulling images from the Application Collection:
+
----
...
----
+
This secret will be used to pull cert-manager container images from the Application Collection registry.
...
# Install the cert-manager Helm chart from the SUSE Application Collection
