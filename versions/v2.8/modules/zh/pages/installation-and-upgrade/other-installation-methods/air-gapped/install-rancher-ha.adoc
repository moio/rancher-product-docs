...
===== 1. 向 SUSE Application Collection 进行身份验证
...
这将下载 Chart 作为一个 `.tgz` 文件。注意：Chart 镜像已配置为从 SUSE Application Collection 镜像仓库拉取，但你需要将这些镜像镜像到你的私有镜像仓库以进行离线安装。请参阅xref:installation-and-upgrade/other-installation-methods/air-gapped/publish-images.adoc[准备私有镜像仓库]文档。
...
. 安装 cert-manager。注意：更新镜像仓库以指向你的私有镜像仓库，你已在其中镜像了 cert-manager 镜像。
+
[,plain]
----
