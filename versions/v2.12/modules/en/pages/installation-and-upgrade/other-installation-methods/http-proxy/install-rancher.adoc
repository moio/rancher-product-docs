...
Create a secret for pulling images from the SUSE Application Collection:
...
Install cert-manager from the SUSE Application Collection. Note that cert-manager also needs your proxy configured in case it needs to communicate with Let's Encrypt or other external certificate issuers:
