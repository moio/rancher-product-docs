...
以下说明使用来自 https://apps.rancher.io/applications/cert-manager[SUSE Application Collection] 的 cert-manager。有关 Application Collection 的更多信息，请参阅 https://docs.apps.rancher.io/[文档]。
...
在安装 cert-manager 之前，你需要向 SUSE Application Collection OCI 镜像仓库进行身份验证。此镜像仓库需要对 Helm 操作和拉取容器镜像进行身份验证。
...
----
+
将 `<USERNAME>` 替换为你的用户名，将 `<TOKEN_OR_SECRET>` 替换为你的访问令牌或服务帐户密钥。有关身份验证的更多信息，请参阅 https://docs.apps.rancher.io/get-started/authentication[SUSE Application Collection 身份验证文档]。
...
----
+
此 Secret 将用于从 Application Collection 镜像仓库拉取 cert-manager 容器镜像。
