...
这会将 Chart 下载为 `.tgz` 文件。注意：Chart 镜像已配置为从 SUSE Application Collection 镜像仓库拉取，但你需要将这些镜像镜像到你的私有镜像仓库以进行离线安装。请参阅 xref:installation-and-upgrade/other-installation-methods/air-gapped/publish-images.adoc[准备私有镜像仓库]文档。
